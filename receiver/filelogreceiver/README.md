...
| `force_flush_period`                | `500ms`                              | Time since last read of data from file, after which currently buffered log should be send to pipeline. The duration must contain the unit of time (e.g.: 200ms, 1s, 1m.) otherwise an error will be caused. Zero means waiting for new data forever.                                               |
...
| `retry_on_failure.initial_interval` | `1s`                                 | Time to wait after the first failure before retrying. The duration must contain the unit of time (e.g.: 200ms, 1s, 1m.) otherwise an error will be caused.                                                                                                                                         |
| `retry_on_failure.max_interval`     | `30s`                                | Upper bound on retry backoff interval. Once this value is reached the delay between consecutive retries will remain constant at the specified value. The duration must contain the unit of time (e.g.: 200ms, 1s, 1m.) otherwise an error will be caused.                                          |
| `retry_on_failure.max_elapsed_time` | `5m`                                 | Maximum amount of time (including retries) spent trying to send a logs batch to a downstream consumer. Once this value is reached, the data is discarded. Retrying never stops if set to `0`. The duration must contain the unit of time (e.g.: 200ms, 1s, 1m.) otherwise an error will be caused. |
