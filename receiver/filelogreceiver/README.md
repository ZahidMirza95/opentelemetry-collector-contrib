...
| `force_flush_period`                | `500ms`                              | Time since last read of data from file, after which currently buffered log should be send to pipeline. The duration can contain the unit of time (e.g.: 200ms, 1s, 1m.), if unit is unspecified it'll default to nanoseconds. Zero means waiting for new data forever. |
...
| `poll_interval`                     | 200ms                                | The duration between filesystem polls. The duration can contain the unit of time (e.g.: 200ms, 1s, 1m.), if unit is unspecified it'll default to nanoseconds.                                                                                                                                                                                                 |
...
| `delete_after_read`                 | `false`                              | If `true`, each log file will be read and then immediately deleted. Requires that the `filelog.allowFileDeletion` feature gate is enabled. This option causes an error if it is used when `start_at` is set to "end".                                                  |
...
| `header`                            | nil                                  | Specifies options for parsing header metadata. Requires that the `filelog.allowHeaderMetadataParsing` feature gate is enabled. See below for details. This option causes an error if it is used when `start_at` is set to "end".                                       |
...
| `retry_on_failure.initial_interval` | `1s`                                 | Time to wait after the first failure before retrying. The duration can contain the unit of time (e.g.: 200ms, 1s, 1m.), if unit is unspecified it'll default to nanoseconds.                                                                                                                                                |
| `retry_on_failure.max_interval`     | `30s`                                | Upper bound on retry backoff interval. Once this value is reached the delay between consecutive retries will remain constant at the specified value. The duration can contain the unit of time (e.g.: 200ms, 1s, 1m.), if unit is unspecified it'll default to nanoseconds.                                                 |
| `retry_on_failure.max_elapsed_time` | `5m`                                 | Maximum amount of time (including retries) spent trying to send a logs batch to a downstream consumer. Once this value is reached, the data is discarded. Retrying never stops if set to `0`. The duration can contain the unit of time (e.g.: 200ms, 1s, 1m.), if unit is unspecified it'll default to nanoseconds.        |
